describe("Add data from file uploads", function () {
    ...
    it("Verify sample plan name error ", function () {
        ...
        cy.findByRole("button", { name: "Error" }).should('have.css', 'background-color', 'rgb(220, 53, 69)');
    ...
    it("Verify Include Custom Sample Types", function () {
        ...
        cy.findByRole('button', { name: 'Next' }).click({ force: true });
        cy.findByRole('button', { name: 'Next' }).click({ force: true });
        cy.findByText('Include Custom Sample Types').should('exist').click({ force: true }).then(() => {
            const sampleOptions = sessionStorage.getItem('tots_user_defined_sample_options');
            const sampleOption = JSON.parse(sampleOptions)[0];
            sessionStorage.setItem('tots_sample_type_selections', JSON.stringify([{ "label": "xyz_sample_name", "value": `${sampleOption?.value}`, "serviceId": "", "status": "add" }]))
        });
        ...
        cy.get('#scenario-description-input').type('cenario_description_input');
        cy.findByRole('button', { name: 'Save' }).click({ force: true });
        cy.findAllByTestId(loadingSpinnerId, { timeout: 200000 }).should("exist");
        cy.findAllByTestId(loadingSpinnerId, { timeout: 200000 }).should("not.exist");

        cy.findByRole("button", { name: "Saved" }).should('have.css', 'background-color', 'rgb(40, 167, 69)');
